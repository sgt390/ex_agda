...
module CorrectByConstruction₂
  ...
  insert : {l : A} {xs xys : List A} → (y : A) → l ≤ y → OPList l xs → y ◂ xs ↝ xys → OPList l xys
  insert x l≤x nil p = cons x nil l≤x p
  insert x l≤x (cons y ys l≤y p₁) p with cmp x y
  ... |    left x≤y     = cons x (cons y ys x≤y p₁) l≤x p
  insert x l≤x (cons y ys l≤y p₁) here | right y≤x = cons y {! cons x ys y≤x  !} l≤y {!   !}
  insert x l≤x (cons y ys l≤y p₁) (there p) | right y≤x = {!   !}
...
-- EXERCISE: Define for any type A together with a relation _≤_ on A a
-- new type "* A" which is A adjoined by a new least element -∞. Use
-- this construction to get rid of the requirement.
