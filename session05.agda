...
lemma x = left (conj-eliminationₗ x)

-- How many witnesses are there of the statement "x ≡ y"?
-- infinite that hold but also infinite that not hold. x ≡ y is not true in general
...
double-negation-elimination f = {!!}   -- this hole cannot be filled

fun : {A B : Set} → ¬ ¬ A → ¬ ¬ B → ¬ ¬ (A × B)
fun = λ z z₁ z₂ → z₁ (λ z₃ → z (λ z₄ → z₂ (z₄ , z₃)))
